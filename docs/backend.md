...
| `pitagora` | 和風ピタゴラ(`OmikujiPitagora.vue`) | 鈴の緒を下に引く | 玉が絵馬ドミノ・鳥居のつづら折り・谷越え・鹿威しを駆け抜け、玉の視点(3D)であみだくじを走って結果の門をくぐる(狐は出ない) |
...
- 前半は横長の 2D の装置(幅 1900)で、カメラが玉を追って横に動く。谷越えは時間の進みを
  遅くしてスローモーションにする。
- 鳥居の中はつづら折りの坂(坂を転がって杭に当たり、下の坂へ落ちて向きを変える)。
  奥行きのある螺旋は 2D の横画面では玉が宙を漂って見えたのでやめた。
- 音の書き文字(「カコーン」「ドン」)は出さない。鹿威しが石を打つ所は石から広がる輪と
  破片と画面の小さな揺れ、太鼓は広がる衝撃の輪で見せる。
- 回転盤で最低1周し、結果が届いていれば玉に飛び込んで 3D の視点に切り替わる。結果が
  遅れたら、届くまで回転盤で回って待つ(25 秒で着地させるのは他の種類と同じ)。
- 3D のあみだは、飛び込む時点(結果が届いた後)で、入る線が結果の門に着くように組み立てる
  (`buildRoute`。中身は隠しあみだと同じ `buildLadder`)。視点に入った時点で全体が見えて
