...
- `cache_data/ranking_cache` が未作成の場合(`rankingUpdate` スケジュール関数が
  ...
  いないため、Go版も同様に internal error(500)として扱う。
...
- **`userOGP`(OGP画像生成)**: `canvas`/`chartjs-node-canvas` によるサーバーサイド
  ...
  他のHTTPエンドポイント(`status`/`sanpai`/`ranking`/`register`/`ogpRewrite`)とは
  技術的な難易度・作業量が大きく異なる。また `userOGP` は生成結果をStorageに
  キャッシュし、ブラウザがOGP画像を非同期に取得する経路(SNSクローラー向け)で
  あり、`ogpRewrite`のようにページの初期表示をブロックしないため、レイテンシー
  改善の優先度は他のエンドポイントより低い。そのため今回は移植を見送り、
  必要になった場合に改めて設計を提案する。
- **スケジュール関数(`rankingUpdate`/`rankingCache`/`statusCacheBackfill`/
  `scheduledOgpDelete`)**: いずれも Pub/Sub(Cloud Scheduler)トリガーで、
  ユーザーがブラウザで結果を待つものではないためコールドスタート短縮の
  恩恵(体感速度の改善)が無い。Go化しても実行時間短縮によるわずかな
  課金削減効果はあり得るが、Pub/SubトリガーのCloud Run functionsをGoで
  デプロイするには専用のCloudEventハンドラ実装とCloud Scheduler/Pub/Sub
  トピックの追加設定が必要で、HTTPトリガー関数とは異なるインフラ変更を伴う。
  今回はユーザーが直接待つHTTPエンドポイントを優先したため対象外とした。
