...
- **せんとうりょく**: `status.total` は現在値しか保存しておらず過去を復元
  できないため、期間開始時点のスナップショット `cache_data/battle_baseline`
  との差分=「期間中に伸びた分」でランキングする。
  - 期間キーが変わったら基準値を作り直す(全員0から再スタート)
  - 基準値に居ないユーザー(新規・初回観測)は現在値で初期化して差分0にする
    (基準値が無い人が現在値まるごとで1位になる事故を防ぐ)
  - 伸び幅0以下は載せない。したがって導入直後は空 → フロントがトータルに
    フォールバックする
  - 居なくなったユーザーの基準値は毎回捨ててドキュメントサイズを抑える
  - **その期間に参拝していないユーザーは載せない**(`sanpaiedSince`)。
    `status.total` はマイページ表示用のキャッシュでもあり、キャッシュが古い
    ユーザーのプロフィールが開かれたとき(`statusGo` の書き戻し)や
    `statusCacheBackfillGo` によって、参拝と無関係に書き直される。単純な差を
    伸び幅にすると、この再計算がそのまま「期間中に伸びた」ことになり、
    何年も参拝していないユーザーが週間・月間ランキングに現れる。
    せんとうりょくは参拝でしか伸びないので、`last_sanpai` が期間内にある
    ユーザーだけを対象にする。基準値そのものは全員分を持ち続ける
    (次に参拝したときの差が正しく出るようにするため)
...
- せんとうりょくの期間ランキング: **毎時**(基準値1read/1writeと差分計算だけ)
