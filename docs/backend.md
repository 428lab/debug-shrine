...
和のテイストのボカロ系の曲に合わせて、鈴・太鼓・柏手の 3 レーンを叩く。ログイン不要、サーバーとは
通信しない。ハイスコアは難易度ごとに端末の localStorage(`debug-shrine:rhythm:best`)にだけ保存する。

- 譜面は `web/components/rhythmChart.js` が曲の音の並びから作る(曲で実際に鳴っている音の時刻に
  置くので、叩くと曲と合う)。鈴 = ボーカル・三味線、太鼓 = キック・和太鼓、柏手 = スネア・クラップ。
  - 参拝(やさしい): 2 拍ごとの頭くらい(平均 2.5 個/秒)。同じレーンは 4 ステップ以上あける
  - 修行(むずかしい): ボーカルのメロディをなぞる(平均 6.9 個/秒)。長いボーカルは長押し
...
- 検証: `web/scripts/test-rhythm-chart.js`(音符が曲の音の時刻にある、レーンの間隔と長押し、同時押し
  2 本まで、修行が参拝より多い、密度、判定と点数)。

#### 曲と音

- 曲は `web/components/rhythmSong.js`(音の並びの純関数)。172 BPM・D マイナーの、和のテイストの
