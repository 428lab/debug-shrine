...
peek はサーバー往復(トークン取得 + `omikujiGo`。関数がコールドなら数秒)を伴うため、
これだけに頼ると**「おみくじ」を開くたびにスピナーを見せられ、待った末に
「まだ引けない」と分かる**。次に引ける時刻は前回引いた瞬間に確定しているので、
覚えておけば待たせる必要が無い。

- `web/utils/omikujiCooldown.js` が次に引ける時刻を localStorage に持つ
  (キーに `github_id` を含め、アカウント切替で前の人の残り時間を見せない)。
  純関数で `storage` を注入でき、`web/scripts/test-omikuji-cooldown.js` で検証する。
- おみくじページはマウント時にこれを読み、残っていれば**通信を待たずに**
  クールダウン表示から始める。peek の応答が来たら上書きする(正はサーバー)。
- ヘッダのリンクは引けないと分かっているとき「前回のおみくじ」になる。
  開く前に、引けるのか前回の結果を見に行くだけなのかが分かる。
- あくまで表示を先出しするだけで、**実際に引けるかは常にサーバーが決める**。
  端末の時計は信用せず、8時間(最長のクールダウン)で頭打ちにする。
