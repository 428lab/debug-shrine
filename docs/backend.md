...
- ベースは Chrome の通信できない時の恐竜ゲーム。ダチョウが走り続け、地面でタップ(スペース
  ...
  カラス(低い = 跳ぶ / 高い = 走り抜ける)、鳥居の門(柱のすき間を羽ばたいてくぐると +20)。
  進むほど速くなり、スコア 1000 ごとに昼と夜が入れ替わる。
...
- ハイスコアは DB に記録せず、端末の localStorage(`debug-shrine:ostrich:best`)にだけ保存する。
  終わると、スコア・称号・ベスト・日付・サイト名の札をゲーム画面の下に出す(スマホの縦画面でも
...
- 障害物の間隔は秒で決める(距離 = 速さ × 秒)。ジャンプの滞空(約 0.7 秒)の後、着地して次に
  跳ぶまでの余裕がどの速さでも残る(距離で決めていた時は、最高速で人の反応が間に合わなかった)。
- タブを離れると一時停止し、タップで再開する。走っている間は「はじめる」ボタンを出さない。
- 検証は `web/scripts/test-ostrich-game.js`。単純な自動操縦と、反応を 60ms 遅らせた自動操縦で
  それぞれ 300 回 × 90 秒走らせ、ほぼ全部走り切れること(= 人の反応でもよけようのない配置が
  出ないこと)と、障害物の間隔を確かめる。
