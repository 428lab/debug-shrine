...
- セクション見出しは `.section-title`(1.35rem / 700)+ 絵文字1つ
  (例: 「🏆 ランキング」「📊 GitHubの実績」)。見出しの絵文字は装飾として許可
...
- **アイコンはFontAwesome単色**(`fas fa-fw fa-*`)で文字色に追従させる。
  UI部品(ボタン・タブ・リスト)内で絵文字を使わない(端末依存の見た目になる。
  #182で ⚔️🪙 → fa-fist-raised / fa-coins に置換した経緯)。
  絵文字が許されるのはセクション見出しの飾りと文章内のみ
...
- ❌ UI部品内に絵文字を置かない(FontAwesome単色を使う)
