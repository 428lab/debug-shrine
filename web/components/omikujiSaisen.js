...
const GEO = {
  ...
  RAIL: { y: 140, minX: 70, maxX: 410, startX: 240 },
...
function buildWorld(Matter, opts) {
  ...
  for (let row = 0; row < P.rows; row++) {
    ...
    for (let x = P.edge + offset; x <= GEO.W - P.edge; x += P.pitch) {
      ...
      // 風車の回る範囲(腕+釘+賽銭1枚ぶん)には釘を置かない。重なっていると
      // 腕が釘をすり抜けて見え、腕と釘の間に賽銭が挟まる元にもなる。
      const nearMill = GEO.WINDMILLS.some(
        (w) => Math.hypot(x + jx - w.x, y + jy - w.y) < w.arm + P.r + GEO.COIN.r * 2
      );
...
function createRitual(Matter, built) {
  ...
  function hold(x) {
    ...
    // ドラッグ中に MouseConstraint が溜めた拘束の衝撃も消す。残すと、下へ
    // 引っ張ってから放した時に、賽銭が上の数段の釘をすり抜けて飛んだ。
    coin.constraintImpulse.x = 0;
    coin.constraintImpulse.y = 0;
    coin.constraintImpulse.angle = 0;
  }
