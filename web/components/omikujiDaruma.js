...
function buildWorld(Matter) {
  ...
  function setSwing(theta, omega) {
    ...
    // ドラッグ中に MouseConstraint が溜めた拘束の衝撃(ウォームスタート用)も消す。
    // 残すと、放した次の更新で指の方向へ蹴り出される。50° を越えて引いて
    // 放すと木槌が一回転して積みの上に乗り、止まってしまった(48通り中12通り)。
    hammer.constraintImpulse.x = 0;
    hammer.constraintImpulse.y = 0;
    hammer.constraintImpulse.angle = 0;
  }
...
function nudge(Matter, built) {
  const { head, state, setSwing } = built;
  // 胴を抜いている途中で止まっていたら、木槌を振り直す(頭は積みの上に
  // 留めているので、頭を押しても動かない)。左へ持ち上げて離せば、真下を
  // 通るときにポンプが一定の強さで打つ。
  if (state.released && !state.parked) {
    setSwing(GEO.HAMMER.maxPullDeg * DEG * 0.6, 0);
    return;
  }
